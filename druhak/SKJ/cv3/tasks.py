...
class GameOfLife:
    ...
    def _alive_neighbors(self, i, j):
        """
        Helper function that counts the number of alive neighbors for a cell at position (i, j).
        """
        alive_neighbors = 0
        for x in range(-1, 2):
            for y in range(-1, 2):
    ...
    def move(self):
        ...
        for i in range(len(self.board)):
            ...
            for j in range(len(self.board[i])):
                alive_neighbors = self._alive_neighbors(i, j)
                if self.board[i][j] == 'x':
