...
template <typename T, std::enable_if_t<not std::is_trivially_destructible_v<T>, bool> = true>
...
// Nowadays useless, because compiler can optimize empty for loops...
template <typename T, std::enable_if_t<std::is_trivially_destructible_v<T>, bool> = true>
void destruct(T*, T*) {
    // no-op
}
...
int main() {
    ...
    fp::tuple<int, float, std::string> tuple {1, 3.14, "string"};
