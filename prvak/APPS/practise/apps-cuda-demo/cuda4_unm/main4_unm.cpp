...
int main( int t_numarg, char **t_arg )
{
    ...
    cv::Mat l_bw_cv_img( l_bgr_cv_img.size()/2, CV_8U );

    // data for CUDA
    CudaImg l_bgr_cuda_img(l_bgr_cv_img.size().height, l_bgr_cv_img.size().width, ( uchar3 * ) l_bgr_cv_img.data), l_bw_cuda_img(l_bgr_cv_img.size().height/2, l_bgr_cv_img.size().width/2, ( uchar3 * ) l_bw_cv_img.data);
