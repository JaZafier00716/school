...
__global__ void kernel_rotate90_image(CudaImg t_cu_img, CudaImg t_cu_img_rotated, int t_direction)
{
    ...
    uchar3 pixel = t_cu_img.m_p_uchar3[l_y * t_cu_img.m_size.x + l_x];

    if (t_direction == 0)
    {
        // Rotate 90° clockwise
        t_cu_img_rotated.at3(l_x, t_cu_img.m_size.y - l_y - 1, pixel);
    }
    else
    {
        // Rotate 90° counterclockwise
        t_cu_img_rotated.at3(t_cu_img.m_size.x - l_x - 1, l_y, pixel);
    }
}
...
__global__ void kernel_scale_image(CudaImg t_cu_orig, CudaImg t_cu_scaled, float2 l_scale)
{
    ...
    uchar3 bgr00 = t_cu_orig.m_p_uchar3[y * t_cu_orig.m_size.x + x];
    uchar3 bgr01 = t_cu_orig.m_p_uchar3[y * t_cu_orig.m_size.x + (1 + x)];
    uchar3 bgr10 = t_cu_orig.m_p_uchar3[(1 + y) * t_cu_orig.m_size.x + x];
    uchar3 bgr11 = t_cu_orig.m_p_uchar3[(1 + y) * t_cu_orig.m_size.x + (1 + x)];
...
__global__ void kernel_character_image(CudaImg t_cu_img, char t_char, int2 pos, uchar3 color)
{
    int l_y = blockDim.y * blockIdx.y + threadIdx.y;
    int l_x = blockDim.x * blockIdx.x + threadIdx.x;
    ...
    if(font[(int)t_char][l_y] & (1 << l_x)) {
        t_cu_img.at3(l_y+pos.y, l_x+pos.x, color);
    }
}

void cu_character(CudaImg t_cu_img, char t_char, int2 pos, uchar3 color)
{
    cudaError_t l_cerr;
