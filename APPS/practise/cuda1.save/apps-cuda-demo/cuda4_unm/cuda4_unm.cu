...
__global__ void kernel_insert_image(CudaImg t_cuda_big_img, CudaImg t_cuda_small_img, int2 t_pos)
{
    // X,Y coordinates and check image dimensions
    int l_y = blockDim.y * blockIdx.y + threadIdx.y;
    int l_x = blockDim.x * blockIdx.x + threadIdx.x;
...
void cu_insert_image(CudaImg &t_cuda_big_img, CudaImg &t_cuda_small_img, int2 t_pos)
{
    ...
    dim3 l_threads(l_block_size, l_block_size);
    kernel_insert_image<<<l_blocks, l_threads>>>(t_cuda_big_img, t_cuda_small_img, t_pos);
